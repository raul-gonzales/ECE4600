...
// Wifi Credentials
#define WIFI_SSID "Stone of the Alver" // SSID of router
#define WIFI_PASSWORD "gnzls864"     // WIFI password
// Database Credentials
#define DATABASE_URL "https://g12-iot-air-water-monitor-db-default-rtdb.firebaseio.com/"
#define API_KEY "AIzaSyA3MAwAUb_o8aYTCNgFI1cbEwnvRc1E9XM"
#define DATABASE_SECRET "Ege33VVGCbeVFUrh0IhdVnSzN2FqvmDRcTD2wXSi"
// UserID
#define USER_EMAIL "ece4600g12@gmail.com"
#define USER_PASSWORD "g12ece4600"
...
void loop()
{
  ...
  if (!systemIsAutomated && (millis()- delayMillis > 1000))
  {
    ...
    if (valveIsOpen)
    {
      ...
      //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<INSERT OPEN SOLENOID VALVE CODE HERE
    }
    else if (!valveIsOpen)
    {
      Serial.println("Valve closed");
      //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<INSERT CLOSE SOLENOID VALVE CODE HERE
    }
  }
